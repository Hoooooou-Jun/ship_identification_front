...
export default class ImgViewer extends Component{
    ...
    componentDidMount(){
        this.setState({
            img_id: this.props.navigation.getParam('id'),
                    ...
                    requestPermission(token).then((response) => {
                        if(response.data.data.user_level > 1){
                            if(this.state.flag == 'Normal'){
                                deleteCommonShipImage(token, this.state.id).then((response)=>{
                                ...
                                deleteWastedShipImage(token, this.state.id).then((response)=>{
                ...
                <base.Content padder>
                    <Modal visible={true} transparent={true} >
                        ...
                        <base.Form style={{flexDirection: 'column', height: 100}}> 
                            <base.Button style={{flex: 1, width: '100%', justifyContent: 'center', backgroundColor: '#006eee', height: 50, elevation: 6,}}
                                onPress={this.updateShipMImage}>
                                <base.Text>대표사진 설정하기</base.Text>
                            </base.Button>
