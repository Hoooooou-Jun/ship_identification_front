...
const TrainCommonShipGallery = (props) => {
    ...
    const requestTrainImage = () => {
		if(trainRequest.length < 30) {
			Alert.alert(
				'선박확인체계 알림',
				'30장 이상의 사진을 선택해주시기 바랍니다.'
			)
		} else {
			const form = trainRequest
			requestTrainImage(props.token, id, form).then((res) => {
				if(res.status == 200) {
	...
	return (
		<base.Container>
			<base.Header style={{backgroundColor: 'white'}}>
				...
				<base.Right>
					<base.Button transparent onPress={requestTrainImage}>
						<base.Form style={{}}>
